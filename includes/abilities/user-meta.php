 ...
 * (oversio_validate_user_meta_key). The hard-block is CVE-class — session tokens, application
...
add_filter( 'oversio_abilities_registry', 'oversio_register_user_meta_definitions' );
 ...
 */
function oversio_register_user_meta_definitions( array $registry ): array {
	$registry['oversio/get-user-meta']    = array(
		...
		'risk'         => 'read',
		'subject'      => 'users',
		'args_builder' => 'oversio_args_get_user_meta',
	);
	$registry['oversio/update-user-meta'] = array(
		...
		'risk'         => 'write',
		'subject'      => 'users',
		'args_builder' => 'oversio_args_update_user_meta',
	);
	$registry['oversio/delete-user-meta'] = array(
		...
		'risk'         => 'destructive',
		'subject'      => 'users',
		'args_builder' => 'oversio_args_delete_user_meta',
	);
	return $registry;
}
 ...
 * @return bool
 */
function oversio_can_access_user_meta( array $input ): bool {
	$id = isset( $input['user_id'] ) ? absint( $input['user_id'] ) : 0;
	if ( $id < 1 || ! current_user_can( 'edit_user', $id ) ) {
		return false;
	}
	$key = isset( $input['key'] ) ? (string) $input['key'] : '';
	return ! is_wp_error( oversio_validate_user_meta_key( $key ) );
}

/**
 * Args for oversio/get-user-meta.
 *
 * @return array<string,mixed>
 */
function oversio_args_get_user_meta(): array {
	return array(
		'label'               => oversio_ability_label( 'oversio/get-user-meta' ),
		'description'         => oversio_ability_description( 'oversio/get-user-meta' ),
		...
		'execute_callback'    => 'oversio_exec_get_user_meta',
		'permission_callback' => 'oversio_perm_user_meta_access',
		'meta'                => array(
			'annotations' => array(
				'readonly'    => true,
				'destructive' => false,
...
/**
 * Args for oversio/update-user-meta.
 *
 * @return array<string,mixed>
 */
function oversio_args_update_user_meta(): array {
	return array(
		'label'               => oversio_ability_label( 'oversio/update-user-meta' ),
		'description'         => oversio_ability_description( 'oversio/update-user-meta' ),
		...
		'execute_callback'    => 'oversio_exec_update_user_meta',
		'permission_callback' => 'oversio_perm_user_meta_access',
		'meta'                => array(
			'annotations' => array(
				'readonly'    => false,
...
/**
 * Args for oversio/delete-user-meta.
 *
 * @return array<string,mixed>
 */
function oversio_args_delete_user_meta(): array {
	return array(
		'label'               => oversio_ability_label( 'oversio/delete-user-meta' ),
		'description'         => oversio_ability_description( 'oversio/delete-user-meta' ),
		'category'            => 'oversio-writes',
		'input_schema'        => array(
			'type'                 => 'object',
			'properties'           => array(
		...
		'execute_callback'    => 'oversio_exec_delete_user_meta',
		'permission_callback' => 'oversio_perm_user_meta_access',
		'meta'                => array(
			'annotations' => array(
				'readonly'    => false,
 ...
 * @return bool
 */
function oversio_perm_user_meta_access( array $input ): bool {
	return oversio_can_access_user_meta( $input );
}

/**
 * Execute oversio/get-user-meta.
 *
 ...
 */
function oversio_exec_get_user_meta( array $input ) {
	$id  = absint( $input['user_id'] );
	$key = oversio_validate_user_meta_key( isset( $input['key'] ) ? (string) $input['key'] : '' );
...
/**
 * Execute oversio/update-user-meta.
 *
 * Re-validates the key, refuses non-scalar values via oversio_sanitize_user_meta_value(),
 ...
 * @return array<string,mixed>|WP_Error
 */
function oversio_exec_update_user_meta( array $input ) {
	$id  = absint( $input['user_id'] );
	$key = oversio_validate_user_meta_key( isset( $input['key'] ) ? (string) $input['key'] : '' );
	if ( is_wp_error( $key ) || ! get_userdata( $id ) instanceof WP_User ) {
		return oversio_generic_error();
	}
	$value = oversio_sanitize_user_meta_value( $key, $input['value'] ?? '' );
...
/**
 * Execute oversio/delete-user-meta.
 *
 * Re-validates the key (defence in depth — the permission callback already gated it),
 ...
 * @return array<string,mixed>|WP_Error
 */
function oversio_exec_delete_user_meta( array $input ) {
	$id  = absint( $input['user_id'] );
	$key = oversio_validate_user_meta_key( isset( $input['key'] ) ? (string) $input['key'] : '' );
	if ( is_wp_error( $key ) || ! get_userdata( $id ) instanceof WP_User ) {
		return oversio_generic_error();
