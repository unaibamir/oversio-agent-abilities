 ...
 */
function oversio_validate_post_type( string $type ) {
	...
	if ( ! oversio_post_type_is_eligible( $type ) ) {
		return new WP_Error( 'oversio_invalid_post_type', __( 'Unsupported post type.', 'oversio-agent-abilities' ) );
	}

	// Then default-deny: only types the operator has explicitly exposed (post/page always-on).
	if ( ! in_array( $type, oversio_allowed_post_types(), true ) ) {
		return new WP_Error( 'oversio_post_type_not_allowed', __( 'This content type is not exposed to agents.', 'oversio-agent-abilities' ) );
	}
 ...
 */
function oversio_post_type_is_eligible( string $type ): bool {
	$type = sanitize_key( $type );
 ...
 * @return list<string>
 */
function oversio_eligible_post_types(): array {
	$types = get_post_types( array(), 'names' );
	return array_values( array_filter( $types, 'oversio_post_type_is_eligible' ) );
}

/**
 * The default-deny post-type allowlist: post/page always-on, every other eligible type
 * opt-in via the oversio_allowed_post_types option (admin selector) or the matching filter.
 *
 ...
 * @return list<string>
 */
function oversio_allowed_post_types(): array {
	$stored = get_option( 'oversio_allowed_post_types', array() );
	$stored = is_array( $stored ) ? array_map( 'sanitize_key', $stored ) : array();

	$allowed = array_merge( array( 'post', 'page' ), $stored );
	$allowed = array_values( array_unique( array_filter( $allowed, 'oversio_post_type_is_eligible' ) ) );
	 ...
	 */
	$allowed = apply_filters( 'oversio_allowed_post_types', $allowed );

	return array_values( array_filter( array_map( 'sanitize_key', (array) $allowed ), 'oversio_post_type_is_eligible' ) );
}
 ...
 * @return list<string>
 */
function oversio_resolve_search_post_types( array $requested ): array {
 ...
 * The oversio_hard_blocked_meta_keys filter may ADD keys; the built-ins are re-merged
 * after it, so a filter can never unblock one.
 *
 * @param string $key Meta key.
 * @return bool
 */
function oversio_hard_blocked_meta_key( string $key ): bool {
	global $wpdb;
	$key = (string) $key;
	if ( '' === trim( $key ) ) {
		return true;
	 ...
	 */
	$extra   = (array) apply_filters( 'oversio_hard_blocked_meta_keys', array() );
...
/**
 * Default-deny meta-key allowlist. Default empty; opt-in via the oversio_allowed_meta_keys
 ...
 * @return list<string>
 */
function oversio_allowed_meta_keys(): array {
	$stored = get_option( 'oversio_allowed_meta_keys', array() );
	$stored = is_array( $stored ) ? array_map( 'strval', $stored ) : array();
	$stored = array_values( array_filter( $stored, static fn( $k ) => ! oversio_hard_blocked_meta_key( $k ) ) );
	 ...
	 */
	$filtered = (array) apply_filters( 'oversio_allowed_meta_keys', $stored );
	...
	// oversio_meta_allow_has_star(), which the validator reads.
	return array_values(
		array_unique(
			array_filter( array_map( 'strval', $filtered ), static fn( $k ) => '' !== $k && '*' !== $k && ! oversio_hard_blocked_meta_key( $k ) )
		)
	);
}
 ...
 *
 * Read from the oversio_denied_meta_keys option, string-coerced, with empties and the `*`
 * sentinel dropped (the star is surfaced separately via oversio_meta_deny_has_star()). Unlike the
 ...
 * @return list<string>
 */
function oversio_denied_meta_keys(): array {
	$stored = get_option( 'oversio_denied_meta_keys', array() );
	$stored = is_array( $stored ) ? array_map( 'strval', $stored ) : array();
 ...
 * @return bool
 */
function oversio_meta_allow_has_star(): bool {
	$raw = get_option( 'oversio_allowed_meta_keys', array() );
 ...
 * @return bool
 */
function oversio_meta_deny_has_star(): bool {
	$raw = get_option( 'oversio_denied_meta_keys', array() );
	return is_array( $raw ) && in_array( '*', array_map( 'strval', $raw ), true );
}
 ...
 */
function oversio_validate_meta_key( string $key ) {
	$key     = trim( (string) $key );
	$exposed = '' !== $key
		&& '*' !== $key                                         // floor 1: the sentinel is never addressable.
		&& ! oversio_hard_blocked_meta_key( $key )                 // floor 1 (absolute).
		&& ! oversio_meta_deny_has_star()                          // floor 2: deny-all kill switch.
		&& ! in_array( $key, oversio_denied_meta_keys(), true )    // floor 2: explicit deny.
		&& ( oversio_meta_allow_has_star() || in_array( $key, oversio_allowed_meta_keys(), true ) ); // floor 3.

	if ( ! $exposed ) {
		return new WP_Error( 'oversio_meta_key_not_allowed', __( 'This meta key is not available to agents.', 'oversio-agent-abilities' ) );
	}
	return $key;
}
 ...
 */
function oversio_sanitize_meta_value( string $key, $value ) {
	if ( ! is_scalar( $value ) ) {
		return new WP_Error( 'oversio_meta_value_invalid', __( 'Only text, number, or boolean meta values are supported.', 'oversio-agent-abilities' ) );
	}
	if ( is_string( $value ) ) {
 ...
 * UNION). Empty by default — opt-in via the oversio_exposed_term_meta_keys option (admin textarea)
 * and/or the oversio_allowed_term_meta_keys filter (Wave-4 SEO integrations populate the filter).
 *
 ...
 * @return list<string>
 */
function oversio_allowed_term_meta_keys(): array {
	$base = get_option( 'oversio_exposed_term_meta_keys', array() );
	 ...
	 */
	$filtered = (array) apply_filters( 'oversio_allowed_term_meta_keys', $base );
	...
	// oversio_term_meta_allow_has_star()), and re-floor the merged set against the hard-block.
	return array_values(
		array_unique(
			array_filter( $merged, static fn( $k ) => '' !== $k && '*' !== $k && ! oversio_hard_blocked_meta_key( $k ) )
		)
	);
}

/**
 * The list of explicitly DENIED term-meta keys. Deny always beats exposed (including exposed-`*`).
 *
 * Read from the oversio_denied_term_meta_keys option, string-coerced, with empties and the `*`
 * sentinel dropped (the star is surfaced separately via oversio_term_meta_deny_has_star()). Does
 * NOT re-floor against the hard-block: denying an already-blocked key is a harmless no-op.
 *
 * @return list<string>
 */
function oversio_denied_term_meta_keys(): array {
	$stored = get_option( 'oversio_denied_term_meta_keys', array() );
	$stored = is_array( $stored ) ? array_map( 'strval', $stored ) : array();
...
/**
 * Whether the term-meta EXPOSED option carries the `*` wildcard (allow-all-but-blocked-or-denied).
 *
 * Reads the RAW option, not the filtered getter (the getter strips the sentinel).
 *
 * @return bool
 */
function oversio_term_meta_allow_has_star(): bool {
	$raw = get_option( 'oversio_exposed_term_meta_keys', array() );
	return is_array( $raw ) && in_array( '*', array_map( 'strval', $raw ), true );
}

/**
 * Whether the term-meta DENY option carries the `*` wildcard (deny-all kill switch).
 *
 * Reads the RAW option, not the filtered getter (the getter strips the sentinel).
 *
 * @return bool
 */
function oversio_term_meta_deny_has_star(): bool {
	$raw = get_option( 'oversio_denied_term_meta_keys', array() );
	return is_array( $raw ) && in_array( '*', array_map( 'strval', $raw ), true );
}

/**
 * Validate a term-meta key against the absolute precedence chain: hard-block -> deny -> allow/`*`.
 *
 * Mirrors oversio_validate_meta_key() but term-scoped (reuses the post-agnostic hard-block floor,
 * oversio_hard_blocked_meta_key). A key is exposed only when ALL hold (hard-block computed FIRST so
 ...
 * @return string|WP_Error
 */
function oversio_validate_term_meta_key( string $key ) {
	$key     = trim( (string) $key );
	$exposed = '' !== $key
		&& '*' !== $key                                              // floor 1: the sentinel is never addressable.
		&& ! oversio_hard_blocked_meta_key( $key )                      // floor 1 (absolute).
		&& ! oversio_term_meta_deny_has_star()                          // floor 2: deny-all kill switch.
		&& ! in_array( $key, oversio_denied_term_meta_keys(), true )    // floor 2: explicit deny.
		&& ( oversio_term_meta_allow_has_star() || in_array( $key, oversio_allowed_term_meta_keys(), true ) ); // floor 3.

	if ( ! $exposed ) {
		return new WP_Error( 'oversio_term_meta_key_not_allowed', __( 'This term meta key is not available to agents.', 'oversio-agent-abilities' ) );
	}
	return $key;
}
 ...
 * @return mixed|WP_Error Sanitized scalar, or error if non-scalar.
 */
function oversio_sanitize_term_meta_value( string $key, $value ) {
	if ( ! is_scalar( $value ) ) {
		return new WP_Error( 'oversio_term_meta_value_invalid', __( 'Only text, number, or boolean term meta values are supported.', 'oversio-agent-abilities' ) );
	}
	if ( is_string( $value ) ) {
		$value = sanitize_text_field( $value );
	}
 ...
 * ({$prefix}_capabilities / {$prefix}2_user_level). The oversio_hard_blocked_user_meta_keys
 ...
 *
 * Mirrors oversio_hard_blocked_meta_key() but is user-scoped (the 'user' protected subtype and
 * the user-specific auth/2FA denylist differ from the post-meta floor).
 *
 * @param string $key Meta key.
 * @return bool
 */
function oversio_hard_blocked_user_meta_key( string $key ): bool {
	global $wpdb;
	$key = (string) $key;
	if ( '' === trim( $key ) ) {
		return true;
	 ...
	 * @param list<string> $extra Extra keys to block.
	 */
	$extra   = (array) apply_filters( 'oversio_hard_blocked_user_meta_keys', array() );
	$blocked = array_merge( $builtin, array_map( 'strval', $extra ) );
	if ( in_array( $key, $blocked, true ) ) {
		return true;
	}
 ...
 * UNION). Empty by default — opt-in via the oversio_exposed_user_meta_keys option (admin textarea)
 * and/or the oversio_allowed_user_meta_keys filter.
 *
 * Merge order (must not change): option base -> filter UNION -> re-floor the merged set against
 ...
 * @return list<string>
 */
function oversio_allowed_user_meta_keys(): array {
	$base = get_option( 'oversio_exposed_user_meta_keys', array() );
	$base = is_array( $base ) ? array_map( 'strval', $base ) : array();
	 ...
	 */
	$filtered = (array) apply_filters( 'oversio_allowed_user_meta_keys', $base );
	$merged   = array_merge( $base, array_map( 'strval', $filtered ) );

	// '*' is a wildcard SENTINEL, never a literal key — strip it here (surfaced separately via
	// oversio_user_meta_allow_has_star()), and re-floor the merged set against the user hard-block.
	return array_values(
		array_unique(
			array_filter( $merged, static fn( $k ) => '' !== $k && '*' !== $k && ! oversio_hard_blocked_user_meta_key( $k ) )
		)
	);
}
 ...
 *
 * Read from the oversio_denied_user_meta_keys option, string-coerced, with empties and the `*`
 * sentinel dropped (the star is surfaced separately via oversio_user_meta_deny_has_star()). Does
 * NOT re-floor against the hard-block: denying an already-blocked key is a harmless no-op.
 *
 * @return list<string>
 */
function oversio_denied_user_meta_keys(): array {
	$stored = get_option( 'oversio_denied_user_meta_keys', array() );
	$stored = is_array( $stored ) ? array_map( 'strval', $stored ) : array();
 ...
 * @return bool
 */
function oversio_user_meta_allow_has_star(): bool {
	$raw = get_option( 'oversio_exposed_user_meta_keys', array() );
	return is_array( $raw ) && in_array( '*', array_map( 'strval', $raw ), true );
}
 ...
 * @return bool
 */
function oversio_user_meta_deny_has_star(): bool {
	$raw = get_option( 'oversio_denied_user_meta_keys', array() );
	return is_array( $raw ) && in_array( '*', array_map( 'strval', $raw ), true );
}
 ...
 * @return list<string>
 */
function oversio_allowed_site_settings(): array {
	 ...
	 */
	$filtered = (array) apply_filters( 'oversio_allowed_site_settings', $base );
 ...
 *
 * Mirrors oversio_validate_meta_key() but user-scoped (the user hard-block + error code differ).
 ...
 * @return string|WP_Error
 */
function oversio_validate_user_meta_key( string $key ) {
	$key     = trim( (string) $key );
	$exposed = '' !== $key
		&& '*' !== $key                                              // floor 1: the sentinel is never addressable.
		&& ! oversio_hard_blocked_user_meta_key( $key )                 // floor 1 (absolute).
		&& ! oversio_user_meta_deny_has_star()                          // floor 2: deny-all kill switch.
		&& ! in_array( $key, oversio_denied_user_meta_keys(), true )    // floor 2: explicit deny.
		&& ( oversio_user_meta_allow_has_star() || in_array( $key, oversio_allowed_user_meta_keys(), true ) ); // floor 3.

	if ( ! $exposed ) {
		return new WP_Error( 'oversio_user_meta_key_not_allowed', __( 'This user meta key is not available to agents.', 'oversio-agent-abilities' ) );
	}
	return $key;
}
 ...
 *
 * Mirrors oversio_sanitize_term_meta_value() but is user-scoped — the live
 * oversio_sanitize_meta_value() hardwires the 'post' subtype and CANNOT be reused here.
 *
 * @param string $key   User-meta key (already validated/allowlisted by the caller).
 * @param mixed  $value Raw value from input.
 * @return mixed|WP_Error Sanitized scalar, or error if non-scalar.
 */
function oversio_sanitize_user_meta_value( string $key, $value ) {
	if ( ! is_scalar( $value ) ) {
		return new WP_Error( 'oversio_user_meta_value_invalid', __( 'Only text, number, or boolean user meta values are supported.', 'oversio-agent-abilities' ) );
	}
	if ( is_string( $value ) ) {
		$value = sanitize_text_field( $value );
	}
 ...
 */
function oversio_validate_term_meta_request( array $input ) {
	...
	$key = oversio_validate_term_meta_key( isset( $input['meta_key'] ) ? (string) $input['meta_key'] : '' );
	if ( is_wp_error( $key ) ) {
		return $key;
	}
	return $taxonomy;
 ...
 */
function oversio_type_caps( string $type ): array {
	$obj = get_post_type_object( $type );
	return array(
 ...
 * @return bool
 */
function oversio_can_read_post_object( WP_Post $post ): bool {
	if ( is_wp_error( oversio_validate_post_type( $post->post_type ) ) ) {
		return false;
	}
 ...
 * @return bool
 */
function oversio_can_edit_post_object( WP_Post $post ): bool {
	$caps = oversio_writable_type_caps( $post );
 ...
 * @return bool
 */
function oversio_can_delete_post_object( WP_Post $post ): bool {
	$caps = oversio_writable_type_caps( $post );
 ...
 */
function oversio_writable_type_caps( WP_Post $post ): ?WP_Post_Type {
	if ( is_wp_error( oversio_validate_post_type( $post->post_type ) ) ) {
		return null;
	}
	$caps = oversio_type_caps( $post->post_type );
 ...
 */
function oversio_validate_taxonomy( string $taxonomy ) {
	$taxonomy = sanitize_key( $taxonomy );
	$allowed  = get_taxonomies( array( 'public' => true ), 'names' );
	if ( ! in_array( $taxonomy, $allowed, true ) ) {
		return new WP_Error( 'oversio_invalid_taxonomy', __( 'Unsupported taxonomy.', 'oversio-agent-abilities' ) );
	}
	return $taxonomy;
}
 ...
 *
 * Enforces, in order: the taxonomy clears the public allow-list (oversio_validate_taxonomy);
 ...
 */
function oversio_validate_term_ids_for_taxonomy( string $taxonomy, array $term_ids ) {
	$tax = oversio_validate_taxonomy( $taxonomy );
	if ( is_wp_error( $tax ) ) {
		return $tax;
	}
 ...
 * oversio_can_edit_post_object), and set_post_thumbnail writes _thumbnail_id on that
 ...
 */
function oversio_validate_featured_attachment_id( $attachment_id ) {
 ...
 *
 * Each key passes oversio_validate_meta_key() (allow-list AND hard-block floor); each value
 * passes oversio_sanitize_meta_value() (scalar-only). Returns a key => sanitized-value map
 ...
 * collapse to the same non-distinguishing generic error (mirrors oversio_exec_update_post_meta
 ...
 */
function oversio_validate_meta_payload( array $meta ) {
	$clean = array();
	foreach ( $meta as $raw_key => $raw_value ) {
		$key = oversio_validate_meta_key( (string) $raw_key );
		if ( is_wp_error( $key ) ) {
			return oversio_generic_error();
		}
		$value = oversio_sanitize_meta_value( $key, $raw_value );
 ...
 * oversio_apply_write_enrichment(), or the first WP_Error. Absent fields are normalized to
 ...
 */
function oversio_validate_write_enrichment( array $input ) {
	$bundle = array(
		'terms'          => array(),
		'featured_media' => 0,
		'meta'           => array(),
	...
	if ( isset( $input['terms'] ) && is_array( $input['terms'] ) ) {
		foreach ( $input['terms'] as $taxonomy => $term_ids ) {
			$ids = oversio_validate_term_ids_for_taxonomy( (string) $taxonomy, is_array( $term_ids ) ? $term_ids : array() );
			if ( is_wp_error( $ids ) ) {
				return $ids;
			}
	...
	if ( isset( $input['featured_media'] ) ) {
		$att = oversio_validate_featured_attachment_id( $input['featured_media'] );
		if ( is_wp_error( $att ) ) {
			return $att;
		}
	...
	if ( isset( $input['meta'] ) && is_array( $input['meta'] ) ) {
		$meta = oversio_validate_meta_payload( $input['meta'] );
		if ( is_wp_error( $meta ) ) {
			return $meta;
		}
		$bundle['meta'] = $meta;
 ...
 * the bundle was already checked by oversio_validate_write_enrichment(), the apply cannot fail
 ...
 */
function oversio_apply_write_enrichment( int $post_id, array $bundle ) {
 ...
 */
function oversio_validate_post_status( string $status, bool $can_read_private ) {
	...
	return new WP_Error( 'oversio_invalid_status', __( 'Unsupported or unauthorized post status.', 'oversio-agent-abilities' ) );
}
 ...
 */
function oversio_redact_post( WP_Post $post ): array {
	return array(
		'id'           => (int) $post->ID,
 ...
 */
function oversio_post_terms_grouped( WP_Post $post ): array {
	$grouped    = array();
 ...
 * @return array<string,mixed>
 */
function oversio_rich_post_output_properties(): array {
	return array(
 ...
 * @return array<string,mixed>
 */
function oversio_rich_post( WP_Post $post, array $options = array() ): array {
	$shape = oversio_redact_post( $post );
	...
	// single-post read gate (oversio_can_read_post_object) admits any public-status
	...
	$shape['terms'] = oversio_post_terms_grouped( $post );
	...
	$meta = array();
	foreach ( oversio_allowed_meta_keys() as $meta_key ) {
 ...
 * @return array<string,mixed>
 */
function oversio_redact_user( $user, ?int $post_count = null ): array {
 ...
 *
 * Intentionally NOT folded into oversio_redact_user(): that stays lean for the get-users
 ...
 * @return array<string,mixed>
 */
function oversio_rich_user( $user, ?int $post_count = null ): array {
	if ( ! $user instanceof WP_User ) {
		return array();
	}
	$base               = oversio_redact_user( $user, $post_count );
 ...
 * @return array<string,mixed>
 */
function oversio_redact_comment( $comment ): array {
 ...
 * @return array<string,mixed>
 */
function oversio_redact_term( WP_Term $term ): array {
	return array(
		'id'          => (int) $term->term_id,
		'name'        => $term->name,
		'slug'        => $term->slug,
 ...
 * @return array<string,mixed>
 */
function oversio_redact_media( WP_Post $attachment ): array {
 ...
 *
 * Intentionally NOT folded into oversio_redact_media(): that redactor stays lean for
 ...
 * @return array<string,mixed>
 */
function oversio_media_item_payload( WP_Post $attachment ): array {
	$base = oversio_redact_media( $attachment );
	$meta = wp_get_attachment_metadata( $attachment->ID );
 ...
 * @return array<string,mixed>
 */
function oversio_redact_revision( WP_Post $revision ): array {
	return array(
		'id'           => (int) $revision->ID,
 ...
 * oversio_redact_revision(): that redactor stays metadata-only for list-revisions, which
 ...
 * content_format=raw, and diff is null. This matches oversio_rich_post()'s chokepoint for
 ...
 * @return array<string,mixed>
 */
function oversio_get_revision_payload( WP_Post $revision, array $input ): array {
	...
	$payload            = oversio_redact_revision( $revision );
 ...
 */
function oversio_validate_revision( int $revision_id, int $post_id ) {
 ...
 */
function oversio_paginate_args( array $input, int $max = 50 ): array {
 ...
 * @return WP_Error
 */
function oversio_generic_error(): WP_Error {
	return new WP_Error( 'oversio_error', __( 'The request could not be completed.', 'oversio-agent-abilities' ) );
}
 ...
 */
function oversio_trash_is_enabled(): bool {
	 ...
	 */
	return (bool) apply_filters( 'oversio_trash_is_enabled', $enabled );
}
 ...
 * @return WP_Error
 */
function oversio_trash_disabled_error(): WP_Error {
	return new WP_Error(
		'oversio_trash_disabled',
 ...
 * @return WP_Post|null
 */
function oversio_get_block_object( int $id ): ?WP_Post {
	$post = get_post( $id );
 ...
 * @return array<string,mixed>
 */
function oversio_redact_block( $block ): array {
 ...
 *
 * Intentionally NOT folded into oversio_redact_block(): the list stays lean. The content is the
 * stored block markup (Gutenberg delimiters preserved) — the point of a reusable block.
 *
 * @param WP_Post|null $block Block post.
 * @return array<string,mixed>
 */
function oversio_rich_block( $block ): array {
	if ( ! $block instanceof WP_Post ) {
		return array();
	}
	$base            = oversio_redact_block( $block );
 ...
 * Mirrors what oversio_rich_block() actually returns — every key, with its type — so the
 ...
 * @return array<string,mixed>
 */
function oversio_rich_block_output_properties(): array {
	return array(
		'id'       => array( 'type' => 'integer' ),
		'title'    => array( 'type' => 'string' ),
		'slug'     => array( 'type' => 'string' ),
 ...
 * @return array<string,mixed>
 */
function oversio_redact_menu( $menu ): array {
 ...
 * Mirrors what oversio_redact_menu() returns — every key, with its type — so the list-menus
 * output_schema can describe its array element shape instead of leaving it open.
 *
 * @return array<string,mixed>
 */
function oversio_menu_output_properties(): array {
	return array(
		'id'    => array( 'type' => 'integer' ),
		'name'  => array( 'type' => 'string' ),
		'slug'  => array( 'type' => 'string' ),
 ...
 * @return array<string,mixed>
 */
function oversio_redact_menu_item( $item ): array {
	if ( ! is_object( $item ) ) {
		return array();
	}
	return array(
 ...
 * Mirrors what oversio_redact_menu_item() returns — every key, with its type — so the
 * list-menu-items output_schema can describe its array element shape instead of leaving it open.
 *
 * @return array<string,mixed>
 */
function oversio_menu_item_output_properties(): array {
	return array(
		'id'        => array( 'type' => 'integer' ),
		'title'     => array( 'type' => 'string' ),
		'url'       => array( 'type' => 'string' ),
 ...
 * Mirrors what oversio_redact_template() returns — every key, with its type — so the list-templates
 * output_schema can describe its array element shape instead of leaving it open.
 *
 * @return array<string,mixed>
 */
function oversio_template_output_properties(): array {
	return array(
 ...
 * @return array<string,mixed>
 */
function oversio_rich_template_output_properties(): array {
	$base            = oversio_template_output_properties();
