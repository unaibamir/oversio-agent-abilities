...
final class ActivityLogTest extends TestCase {
	...
	private function register_activity_log(): void {
		...
		update_option( 'oversio_enabled_abilities', array( 'oversio/get-activity-log' ) );
	...
	public function test_requires_manage_options(): void {
		...
		$this->assertNotTrue(
			wp_get_ability( 'oversio/get-activity-log' )->check_permissions( array() ),
	...
	public function test_returns_rows_most_recent_first_without_source_ip(): void {
		...
		$res = wp_get_ability( 'oversio/get-activity-log' )->execute( array() );
		...
		$i_pages = array_search( 'oversio/get-pages', $order, true );
		$i_posts = array_search( 'oversio/get-posts', $order, true );
	...
	public function test_status_and_ability_filters_pass_through(): void {
		...
		$res = wp_get_ability( 'oversio/get-activity-log' )->execute( array( 'status' => 'denied' ) );
		$this->assertCount( 1, $res['entries'] );
		$this->assertSame( 'oversio/trash-post', $res['entries'][0]['ability'] );
	}

	public function test_rejects_a_smuggled_field(): void {
		$this->acting_as( 'administrator' );
		$res = wp_get_ability( 'oversio/get-activity-log' )->execute( array( 'principal_user_id' => 5 ) );
	...
	public function test_denial_is_audited(): void {
		...
		wp_get_ability( 'oversio/get-activity-log' )->execute( array() );
		...
		$this->assertContains( 'oversio/get-activity-log', $abilities, 'a denied read must be audited.' );
	}
}
