...
final class UserMetaTest extends TestCase {
	...
	public function test_auth_keys_are_hard_blocked_for_everyone(): void {
		...
		foreach ( $keys as $key ) {
			$this->assertTrue( oversio_hard_blocked_user_meta_key( $key ), "$key must be hard-blocked." );
		}
		// Multisite per-blog forms (wp_2_capabilities / wp_2_user_level) must also block.
		$this->assertTrue( oversio_hard_blocked_user_meta_key( $wpdb->prefix . '2_capabilities' ) );
		$this->assertTrue( oversio_hard_blocked_user_meta_key( $wpdb->prefix . '2_user_level' ) );
		// An empty key is refused.
		$this->assertTrue( oversio_hard_blocked_user_meta_key( '' ) );
		// A protected-meta (underscore) key is refused.
		$this->assertTrue( oversio_hard_blocked_user_meta_key( '_hidden' ) );
		// A plain custom key is NOT hard-blocked.
		$this->assertFalse( oversio_hard_blocked_user_meta_key( 'twitter' ) );
	}

	public function test_allowlist_defaults_empty_and_filter_adds(): void {
		$this->assertSame( array(), oversio_allowed_user_meta_keys() );
		add_filter( 'oversio_allowed_user_meta_keys', static fn() => array( 'twitter', 'session_tokens' ) );
		$keys = oversio_allowed_user_meta_keys();
	...
	public function test_capability_keys_need_manage_options_even_when_allowlisted(): void {
		add_filter( 'oversio_allowed_user_meta_keys', static fn() => array( 'wp_capabilities' ) );
		...
		$this->assertTrue( oversio_hard_blocked_user_meta_key( 'wp_capabilities' ) );
		$this->assertNotContains( 'wp_capabilities', oversio_allowed_user_meta_keys() );
		$this->assertWPError( oversio_validate_user_meta_key( 'wp_capabilities' ) );
	}
	...
	public function test_validate_user_meta_key_requires_allowlist_and_not_blocked(): void {
		...
		$this->assertWPError( oversio_validate_user_meta_key( 'twitter' ) );
		add_filter( 'oversio_allowed_user_meta_keys', static fn() => array( 'twitter' ) );
		$this->assertSame( 'twitter', oversio_validate_user_meta_key( 'twitter' ) );
		// Blocked key stays refused even after allowlisting.
		add_filter( 'oversio_allowed_user_meta_keys', static fn( $k ) => array_merge( $k, array( 'session_tokens' ) ) );
		$this->assertWPError( oversio_validate_user_meta_key( 'session_tokens' ) );
	}

	public function test_user_meta_value_sanitizer_is_scalar_only(): void {
		$this->assertSame( 'plain text', oversio_sanitize_user_meta_value( 'twitter', 'plain text' ) );
		$this->assertSame( 7, oversio_sanitize_user_meta_value( 'twitter', 7 ) );
		$this->assertWPError( oversio_sanitize_user_meta_value( 'twitter', array( 'a' => 'b' ) ) );
		$this->assertWPError( oversio_sanitize_user_meta_value( 'twitter', new \stdClass() ) );
	}
	...
	public function test_user_meta_roundtrip_on_allowlisted_key(): void {
		...
		$set = wp_get_ability( 'oversio/update-user-meta' )->execute(
			array(
				'user_id' => $uid,
				'key'     => 'twitter',
				'value'   => '@handle',
		...
		$get = wp_get_ability( 'oversio/get-user-meta' )->execute(
			array(
				'user_id' => $uid,
				'key'     => 'twitter',
			)
		...
		$del = wp_get_ability( 'oversio/delete-user-meta' )->execute(
			array(
				'user_id' => $uid,
				'key'     => 'twitter',
			)
	...
	public function test_user_meta_refuses_blocked_key_even_if_targeted(): void {
		...
		$this->acting_as( 'administrator' );
		$uid = self::factory()->user->create( array( 'role' => 'author' ) );
		$res = wp_get_ability( 'oversio/get-user-meta' )->execute(
			array(
				'user_id' => $uid,
				'key'     => 'session_tokens',
			)
	...
	public function test_user_meta_gate_is_edit_user(): void {
		...
		$this->assertNotTrue( wp_get_ability( 'oversio/update-user-meta' )->check_permissions( array( 'user_id' => $uid ) ) );
		$this->assertNotTrue( wp_get_ability( 'oversio/get-user-meta' )->check_permissions( array( 'user_id' => $uid ) ) );
		$this->assertNotTrue( wp_get_ability( 'oversio/delete-user-meta' )->check_permissions( array( 'user_id' => $uid ) ) );
	}
	...
	public function test_denylist_keys_cannot_be_read_written_or_deleted_even_when_allowlisted(): void {
		...
		add_filter( 'oversio_allowed_user_meta_keys', static fn() => $denied );
		$this->register_all();
		$this->acting_as( 'administrator' );
		$uid = self::factory()->user->create( array( 'role' => 'author' ) );
		...
		foreach ( $denied as $key ) {
			...
			$read = wp_get_ability( 'oversio/get-user-meta' )->execute(
				array(
					'user_id' => $uid,
					'key'     => $key,
				)
			...
			$write = wp_get_ability( 'oversio/update-user-meta' )->execute(
				array(
					'user_id' => $uid,
					'key'     => $key,
					'value'   => 'pwn',
			...
			$delete = wp_get_ability( 'oversio/delete-user-meta' )->execute(
				array(
					'user_id' => $uid,
					'key'     => $key,
				)
	...
	public function test_user_meta_discoverable_by_capable_admin_only(): void {
		...
		$this->acting_as( 'administrator' );
		$this->assertTrue( oversio_user_can_discover_ability( 'oversio/get-user-meta' ) );
		$this->assertTrue( oversio_user_can_discover_ability( 'oversio/update-user-meta' ) );
		$this->assertTrue( oversio_user_can_discover_ability( 'oversio/delete-user-meta' ) );

		$this->acting_as( 'subscriber' );
		$this->assertFalse( oversio_user_can_discover_ability( 'oversio/get-user-meta' ) );
		$this->assertFalse( oversio_user_can_discover_ability( 'oversio/update-user-meta' ) );
		$this->assertFalse( oversio_user_can_discover_ability( 'oversio/delete-user-meta' ) );
	}
}
