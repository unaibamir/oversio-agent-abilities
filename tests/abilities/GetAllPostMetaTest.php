...
final class GetAllPostMetaTest extends TestCase {
	...
	public function test_in_registry_as_a_read(): void {
		...
		$this->assertArrayHasKey( 'oversio/get-all-post-meta', $registry );
		$this->assertSame( 'reads', $registry['oversio/get-all-post-meta']['group'] );
		$this->assertSame( 'read', $registry['oversio/get-all-post-meta']['risk'] );
		$this->assertSame( 'content', $registry['oversio/get-all-post-meta']['subject'] );
	}

	public function test_returns_only_allowlisted_scalars(): void {
		update_option( 'oversio_allowed_meta_keys', array( 'subtitle', 'rating' ) );
		...
		$out  = oversio_exec_get_all_post_meta( array( 'post_id' => $id ) );
		$meta = (array) $out['meta'];
	...
	public function test_skips_hard_blocked_and_non_scalar(): void {
		...
		// rogue filter exposed it, oversio_allowed_meta_keys() strips it, so it cannot appear.
		update_option( 'oversio_allowed_meta_keys', array( 'subtitle', 'blob' ) );
		$author = self::factory()->user->create( array( 'role' => 'author' ) );
		wp_set_current_user( $author );
		$id = self::factory()->post->create( array( 'post_author' => $author ) );
	...
	public function test_denied_for_low_cap_user(): void {
		...
		$this->assertFalse( oversio_perm_get_all_post_meta( array( 'post_id' => $id ) ) );
	}

	public function test_missing_post_is_generic_error(): void {
		update_option( 'oversio_allowed_meta_keys', array( 'subtitle' ) );
		$this->acting_as( 'editor' );
		$out = oversio_exec_get_all_post_meta( array( 'post_id' => 999999 ) );
		$this->assertInstanceOf( \WP_Error::class, $out );
	}

	public function test_perm_callback_returns_false_on_empty_input(): void {
		$this->assertFalse( oversio_perm_get_all_post_meta( array() ) );
	}
