...
final class IntegrationsTabTest extends TestCase {
	...
	public function test_tab_is_registered(): void {
		$this->assertArrayHasKey( 'integrations', oversio_admin_tabs() );
	}
	...
	public function test_tab_renders_the_cards_and_the_disclaimer(): void {
		...
		// oversio_woocommerce_active seam off — the same seam production detection passes through — so
		...
		$this->assertStringContainsString( 'oversio-integrations-disclaimer', $html );
	...
	public function test_inactive_card_shows_the_manifest_count(): void {
		...
		$wc = oversio_integration_manifest()['woocommerce'];
		$this->assertStringContainsString( 'oversio-integration-count', $html );
	...
	public function test_inactive_card_still_renders_its_ability_rows_disabled(): void {
		...
		$this->assertStringContainsString( 'value="oversio/wc-list-products"', $html );
	...
	public function test_status_helper_reports_not_installed_when_host_files_absent(): void {
		...
		// deterministic regardless of test order.
		add_filter( 'oversio_woocommerce_active', '__return_false', 99 );
		$this->assertSame( 'not_installed', oversio_integration_status( 'woocommerce' ) );
		$this->assertSame( 'not_installed', oversio_integration_status( 'yoast' ) );
		remove_filter( 'oversio_woocommerce_active', '__return_false', 99 );
		remove_filter( 'oversio_yoast_active', '__return_false', 99 );
	}
	...
	public function test_status_helper_reports_installed_inactive_when_file_present_but_class_absent(): void {
		...
		// helper would say 'active'. Pin the oversio_acf_active seam off — the same seam production
		...
		$plugin_file = oversio_integration_cards()['acf']['plugins'][0];
		...
		try {
			$this->assertSame( 'installed_inactive', oversio_integration_status( 'acf' ) );
		} finally {
	...
	public function test_status_helper_reports_active_when_forced_on(): void {
		...
		$this->assertSame( 'active', oversio_integration_status( 'woocommerce' ) );
	...
	public function test_each_card_is_a_collapsed_details_accordion(): void {
		...
		$this->assertStringContainsString( '<details class="oversio-card oversio-integration-card', $html );
		$this->assertStringNotContainsString( '<details class="oversio-card oversio-integration-card oversio-integration-woocommerce" open', $html );
		...
		$wc_open = strpos( $html, 'oversio-integration-woocommerce' );
		$this->assertNotFalse( $wc_open, 'The WooCommerce card should render.' );
		$summary_open = strpos( $html, '<summary class="oversio-card-head', $wc_open );
		...
		$this->assertStringContainsString( 'oversio-pill', $summary );
		$this->assertStringContainsString( 'oversio-integration-count', $summary );

		// The status note is re-enabled and renders in the accordion content.
		$this->assertStringContainsString( 'oversio-integration-note', $html );

		// The inner "Abilities X/Y" sub-collapsible is gone — the section accordion replaces it.
		$this->assertStringNotContainsString( 'oversio-abilities-details', $html );
	}
	...
	public function test_each_card_renders_a_search_and_risk_filter(): void {
		...
		$this->assertSame( 5, substr_count( $html, 'oversio-integration-filter' ) );
