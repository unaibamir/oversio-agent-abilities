...
final class DashboardTest extends TestCase {
	...
	public function test_agent_user_candidates_flags_admins(): void {
		...
		$cands = oversio_agent_user_candidates();
	...
	public function test_enabled_count_and_protocol(): void {
		$this->assertIsInt( oversio_enabled_ability_count() );
		$this->assertIsInt( oversio_total_ability_count() );
		$this->assertNotEmpty( oversio_mcp_protocol_version() );
	}

	public function test_activity_count_reflects_rows(): void {
		$this->assertSame( 0, oversio_activity_count() );
		$this->acting_as( 'administrator' );
		oversio_log_activity(
			array(
				'ability' => 'oversio/get-post',
				'status'  => 'success',
			)
		);
		oversio_log_activity(
	...
	public function test_dashboard_shows_abilities_off_state(): void {
		update_option( 'oversio_enabled_abilities', array() );
		ob_start();
		oversio_render_dashboard_tab();
		$html = ob_get_clean();
	...
	public function test_dashboard_warns_when_agent_user_can_manage_site(): void {
		$admin = self::factory()->user->create(
			array(
				'role'       => 'administrator',
				'user_login' => 'oversio-admin-agent',
			)
		);
		...
		$this->assertStringContainsString( 'oversio-pill oversio-pill-warn', $html );
		$this->assertStringContainsString( 'Review role', $html );
		$this->assertStringContainsString( 'oversio-admin-agent', $html );
	}
	...
	public function test_dashboard_renders_setup_checklist_and_stat_grid(): void {
		...
		$this->assertStringContainsString( 'oversio-setup', $html );
		$this->assertStringContainsString( 'oversio-step', $html );
		$this->assertStringContainsString( 'oversio-stat-grid', $html );
	...
	public function test_dashboard_setup_checklist_emits_styling_classes_when_incomplete(): void {
		...
		$this->assertStringContainsString( 'oversio-setup', $html );
		$this->assertStringContainsString( 'oversio-setup-count', $html );
		// Both row states are reachable: at least one done step and the incomplete one.
		$this->assertStringContainsString( 'oversio-step-todo', $html );
		$this->assertStringContainsString( 'oversio-step-done', $html );
		// State pill text is present for the to-do step.
		$this->assertStringContainsString( 'oversio-step-state', $html );
	...
	public function test_dashboard_setup_collapses_when_every_step_is_complete(): void {
		...
		// Steps are always rendered now, inside a collapsible <details class="oversio-setup">.
		$this->assertStringContainsString( '<details class="oversio-setup"', $html );
		$this->assertStringContainsString( 'oversio-step', $html );
		// Complete → collapsed: the <details> carries no open attribute.
		$this->assertStringNotContainsString( 'class="oversio-setup" open', $html );
		...
		$this->assertStringNotContainsString( 'All set', $html );
		$this->assertStringNotContainsString( 'oversio-notice-success', $html );
	}
	...
	public function test_dashboard_setup_stays_open_when_incomplete(): void {
		...
		$this->assertStringContainsString( 'class="oversio-setup" open', $html );
	...
	public function test_has_oauth_grant_false_without_grants(): void {
		$this->assertFalse( oversio_has_oauth_grant() );
	}
